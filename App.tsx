...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          console.error(err);
          setError("Please enable location services to find lunch spots nearby!");
        }
      );
    } else {
  ...
  const handleSearch = async () => {
    ...
    setError(null);
    setPlaces([]);
    setBattleCandidates([]); // Reset battle on new search

    try {
      const result = await findRestaurants(coords, filters);
      setPlaces(result.places);
      if (result.places.length === 0) {
        setError("Couldn't find any places matching those criteria. Try increasing the radius!");
      }
    } catch (err) {
      setError("Oops! The Lunch Bot got confused. Try again later.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleBattleFinish = (winner: Place) => {
      ...
      // Removed: setShowRoulette(true); -> Now we show battleWinner explicitly
  };
    ...
    <div className="min-h-screen pb-32 bg-orange-50 bg-[url('https://www.transparenttextures.com/patterns/food.png')] bg-fixed">
      ...
      <header className="bg-white/90 backdrop-blur-md sticky top-0 z-40 border-b border-orange-100 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          {/* Battle Counter in Header if active */}
          {battleCandidates.length > 0 && (
             <div className="text-sm font-bold text-purple-600 bg-purple-100 px-3 py-1 rounded-full animate-pulse cursor-pointer" onClick={() => setShowBattle(true)}>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 flex flex-col items-center relative">
        ...
        <div className="w-full flex justify-center mb-12">
          {loading ? (
            ...
            >
              {coords ? "FIND SPOTS 🔎" : "Locating You..."}
            </button>
          )}
        </div>
        ...
        {/* Error Message */}
        {error && (
          <div className="p-4 bg-red-50 text-red-600 border border-red-200 rounded-lg mb-8 text-center max-w-md">
            {error}
          </div>
        )}
        ...
        {places.length > 0 && !loading && (
          <div className="w-full animate-fade-in pb-24">
            ...
            <p className="text-center text-sm text-gray-500 mb-6 bg-white/60 py-2 rounded-lg border border-white">
                💡 Tip: Hover over cards and click <strong>"Battle"</strong> to start a race!
            </p>
              ...
              <div className="max-w-4xl mx-auto flex items-center justify-between">
                  <div className="flex items-center gap-4 overflow-x-auto scrollbar-hide">
                      <div className="flex flex-col">
                          ...
                          <span className="font-bold text-purple-600 whitespace-nowrap">{battleCandidates.length} / 4 Racers</span>
                      </div>
